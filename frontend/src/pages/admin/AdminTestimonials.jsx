...
const EMPTY = { name: "", rating: 5, review: "", event_type: "Haldi", image: "", visible: true };
    ...
    >
      ...
      {items.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl">
          ...
          <button onClick={() => setEditing({ ...EMPTY })} className="mt-4 text-[#6B4F8C] underline">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          {items.map((t, idx) => (
            <motion.div
              ...
              className="bg-white rounded-2xl p-5 md:p-6 shadow-md hover:shadow-xl transition-all duration-300"
            >
              {/* Rating Stars */}
              <div className="flex gap-1 mb-3">
                {[...Array(t.rating)].map((_, i) => (
                  <Star key={i} size={14} className="fill-[#BFA2DB] text-[#BFA2DB]" />
                ))}
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="p-2 rounded-lg bg-[#BFA2DB]/20 text-[#6B4F8C] hover:bg-[#BFA2DB]/40 transition active:scale-95"
                  >
                    <Pencil size={14} />
                  </button>
                  <button
                    onClick={() => remove(t.id)}
                    className="p-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition active:scale-95"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
          ...
          >
            ...
            >
              ...
              <div className="p-4 md:p-6 space-y-5">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full border-b-2 border-[#BFA2DB]/40 py-2 focus:outline-none focus:border-[#6B4F8C] bg-white"
                  >
                    {EVENT_TYPES.map((t) => (
              ...
              <div className="sticky bottom-0 bg-white p-4 md:p-6 border-t border-gray-100">
                <button
                  onClick={save}
                  className="w-full bg-[#6B4F8C] text-white py-3 rounded-full hover:bg-[#4F3A6A] transition-all duration-300 hover:scale-[1.02] active:scale-95"
                >
                  Save Testimonial
                </button>
              </div>
